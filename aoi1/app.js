/* 
  ...
  - HTML/CSS/JS 分離版
  - 6ヶ月保存（localStorage）
  - デイリー: 腹筋+腹斜筋=22（筋肉痛で減算）
  - 追加部位: 1部位必須（難易度5/10/20 + random）
  - 達成度: 25回=100%（固定）で超過も表示
  - カレンダー: 一般的な月カレンダー + 日付詳細
  - グラフ: 1日/1週/全体 を棒グラフ + 体重推移
*/

const BASELINE_100 = 25;
...
const DIFFS = [
  { key: 'easy', label: 'easy', base: 5,  rmin: 1,  rmax: 5  },
  { key: 'normal', label: 'normal', base: 10, rmin: 6,  rmax: 10 },
  { key: 'hard', label: 'hard', base: 20, rmin: 11, rmax: 20 },
];

// soreness: core 100から減算（random）
const SORENESS = {
  none:   { label: 'なし',   min: 0, max: 0 },
  sweet:  { label: '厳しめ',   min: 0, max: 2 },
  normal: { label: '普通',   min: 3, max: 5 },
  hard:   { label: '甘め', min: 6, max: 8 },
};
...
function setSoreness(mode) {
  ...
  if (note) {
    note.textContent = (mode === 'none')
      ? '減算なし（22回）'
      : `${conf.label}：22回から ${subtract} 回減らす（random）`;
  }
  renderOption();
}
